// ============================================================================
// GLOWCO SHIPMENTS PLAN DATA — Time-series WOS view with supply walk by SKU
// Generates weekly WOS by channel + expandable supply walk measures
// ============================================================================

import { seededRand } from "../utils/random"
import { FORMULATIONS, PACK_CONFIGS, CHANNELS, WAREHOUSES } from "./glowco-master-data"
import type { Channel } from "./glowco-master-data"
...
const TOP_PACKS = PACK_CONFIGS.slice(0, 2) // Standard + Travel Size
...
export const PLANNERS = ["Sarah M.", "James K."]
...
function generateSkuWalk(
  ...
  const seed = hashStr(formulation.id) * 31 + hashStr(pack.id) * 47 + hashStr(channel.id) * 67 + 1000
  const rng = seededRand(seed)

  const id = `ship-${channel.id}-${formulation.id}-${pack.id}`
  const skuName = `GlowCo ${formulation.name} (${pack.name})`

  // Base weekly demand for this SKU at this channel
  const monthlyUnits = formulation.monthlyDemand * channel.demandShare * pack.demandShare
  const weeklyUnits = monthlyUnits / pack.unitsPerPack / 4.33
  ...
  for (const channel of channels) {
    for (const formulation of FORMULATIONS) {
      for (const pack of TOP_PACKS) {
        skuWalk.push(generateSkuWalk(formulation, pack, channel, periods))
      }
    }
  }
